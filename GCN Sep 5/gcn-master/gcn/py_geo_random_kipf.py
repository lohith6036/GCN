...
with open("test_labels_infomap.txt","rb") as fp:
    ts_labels = pkl.load(fp)
...
# here I do selection of test labels same as louvain/infomap:
test_mask_ids.extend(ts_labels)

with open("training_labels_infomap.txt","rb") as fp:
    trains = pkl.load(fp)

tr_labels = []
for i in all_labels_data:
    if i not in ts_labels:
        tr_labels.append(i)

train_mask_ids = np.random.choice(tr_labels,len(trains),replace = False)
#train_mask_ids.extend(tr_labels[0:int(0.1*len(all_labels_data))

with open("training_labels_Kipf.txt","wb") as fp:
        pkl.dump(train_mask_ids,fp)

'''val_labels = []
for i in all_labels_data:
    if i not in tr_labels:
        if i not in ts_labels:
            val_labels.append(i)'''
val_labels = []
for i in all_labels_data:
    if i not in train_mask_ids:
        if i not in ts_labels:
            val_labels.append(i)

#val_mask_ids.extend(val_labels[int(0.2*len(all_labels_data)):int(0.3*len(all_labels_data))])
val_mask_ids = np.random.choice(val_labels,len(trains),replace = False)
...
#print("Ground Truth : Test Labels ",Counter(data.y[data.Test_Mask].cpu().numpy()))
#print("Train Labels ",Counter(data.y[data.Train_Mask].cpu().numpy()))
#print("Ground Truth -- Test Labels", Counter(labels[test_mask_ids]))
...
print("class labels distribution--training labels",Counter(labels[train_mask_ids]))
print("ground truth of test labels:", Counter(labels[test_mask_ids]))
print("predicted test labels:", Counter(predicted[test_mask_ids]))
...
for i in range(classes):
    ...
    if s2 == 0:
        ...
        rec1 = tp/float (s2)
    f_sc = 2*(prec1*rec1)/float(prec1+rec1)
